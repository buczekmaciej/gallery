 ...
 */
final class Version20191113101846 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__gallery AS SELECT id, category_id, image, likes, views, saves, title, added_at FROM gallery');
        $this->addSql('DROP TABLE gallery');
        $this->addSql('CREATE TABLE gallery (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, category_id INTEGER NOT NULL, image BLOB NOT NULL, likes INTEGER NOT NULL, views INTEGER NOT NULL, saves INTEGER NOT NULL, title VARCHAR(300) DEFAULT NULL COLLATE BINARY, added_at DATETIME DEFAULT NULL, CONSTRAINT FK_472B783A12469DE2 FOREIGN KEY (category_id) REFERENCES categories (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO gallery (id, category_id, image, likes, views, saves, title, added_at) SELECT id, category_id, image, likes, views, saves, title, added_at FROM __temp__gallery');
        $this->addSql('DROP TABLE __temp__gallery');
        $this->addSql('CREATE INDEX IDX_472B783A12469DE2 ON gallery (category_id)');
        $this->addSql('ALTER TABLE user ADD COLUMN is_disabled BOOLEAN DEFAULT NULL');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__gallery AS SELECT id, category_id, image, likes, views, saves, title, added_at FROM gallery');
        $this->addSql('DROP TABLE gallery');
        $this->addSql('CREATE TABLE gallery (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, category_id INTEGER NOT NULL, image BLOB NOT NULL, likes INTEGER NOT NULL, views INTEGER NOT NULL, saves INTEGER NOT NULL, title VARCHAR(300) DEFAULT NULL, added_at DATETIME DEFAULT NULL)');
        $this->addSql('INSERT INTO gallery (id, category_id, image, likes, views, saves, title, added_at) SELECT id, category_id, image, likes, views, saves, title, added_at FROM __temp__gallery');
        $this->addSql('DROP TABLE __temp__gallery');
        $this->addSql('CREATE INDEX IDX_472B783A12469DE2 ON gallery (category_id)');
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, login, password, email, reset_hash, color_schema FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, login VARCHAR(50) NOT NULL, password VARCHAR(50) NOT NULL, email VARCHAR(255) NOT NULL, reset_hash VARCHAR(100) NOT NULL, color_schema VARCHAR(5) DEFAULT NULL)');
        $this->addSql('INSERT INTO user (id, login, password, email, reset_hash, color_schema) SELECT id, login, password, email, reset_hash, color_schema FROM __temp__user');
